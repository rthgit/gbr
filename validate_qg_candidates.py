#!/usr/bin/env python3
"""
validate_qg_candidates.py - Critical Validation of QG Candidates

Usage:
  python validate_qg_candidates.py

Outputs:
  - validation_results.csv (with p_value, p_value_emp, pval_fdr, reject_fdr)
  - plots/ directory with top-N Δt vs E scatter plots
...
def simulate_photon_data_for_validation(source_id, n_photons, has_qg_effect):
    """Simulate photon data for validation (since we don't have real photon-level data)"""
...
def empirical_pvalue(energies, times, nperm=10000):
...
def critical_validation_analysis(df):
    ...
    for idx, row in df.iterrows():
        ...
        p_emp, r_obs = empirical_pvalue(energies, times, nperm=10000)
...
def create_validation_plots(results, top_n=20):
...
def generate_critical_report(results):
    ...
    report = {
        ...
        'critical_conclusions': [
            f"FDR correction at α=0.05 rejects only {sources_rejected_fdr}/{total_sources} sources ({sources_rejected_fdr/total_sources:.1%})",
            f"Mean significance of {np.mean(significances):.2f}σ is well below 3σ threshold",
            f"Only {sum(1 for s in significances if s > 3.0)} sources exceed 3σ significance",
...
def main():
    ...
    create_validation_plots(results, top_n=20)
    ...
    print(f"Sources rejected by FDR (α=0.05): {report['validation_summary']['sources_rejected_fdr_05']}")
    print(f"FDR rejection rate: {report['validation_summary']['fdr_rejection_rate']:.1%}")
    print(f"Mean significance: {report['significance_analysis']['mean_significance']:.2f}σ")
    print(f"Sources above 3σ: {report['significance_analysis']['sources_above_3sigma']}")
