#!/usr/bin/env python3
"""
Analisi DATI REALI Fermi LAT per GRB080916C
...
def load_real_fermi_events(fits_file):
    ...
    if not os.path.exists(fits_file):
        ...
        print("📥 Scarica il file con wget:")
        print("wget https://fermi.gsfc.nasa.gov/FTP/fermi/data/lat/queries/L251020154246F357373F64_EV00.fits")
        return None
    ...
    try:
        ...
        with fits.open(fits_file) as hdul:
            ...
            # Carica eventi (di solito nella prima HDU)
            events_data = hdul[0].data
            
            print(f"Numero totale eventi: {len(events_data)}")
            print(f"Colonne disponibili: {events_data.dtype.names}")
            
            # Estrai colonne principali
            # Note: i nomi delle colonne possono variare
            if 'TIME' in events_data.dtype.names:
                times = events_data['TIME']
            elif 'MET' in events_data.dtype.names:
                times = events_data['MET']
            else:
                print("❌ Colonna TIME non trovata!")
                return None
            
            if 'ENERGY' in events_data.dtype.names:
                energies = events_data['ENERGY']
            else:
                print("❌ Colonna ENERGY non trovata!")
                return None
