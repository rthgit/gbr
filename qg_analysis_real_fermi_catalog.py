#!/usr/bin/env python3
"""
QG Analysis on Real Fermi LAT Catalog Data
Analyze unassociated sources for QG effects
"""
...
def analyze_qg_effects(df_results):
    ...
    print(f"\nHigh significance sources (σ > 3): {len(high_sig)} ({len(high_sig)/total_sources:.1%})")
    print(f"Very high significance sources (σ > 5): {len(very_high_sig)} ({len(very_high_sig)/total_sources:.1%})")
    ...
    if len(qg_data) > 0:
        ...
        print(f"  Mean significance: {qg_data['Significance_Sigma'].mean():.2f}σ")
        print(f"  Max significance: {qg_data['Significance_Sigma'].max():.2f}σ")
    
    if len(no_qg_data) > 0:
        print(f"\nNo QG Effect Sources:")
        print(f"  Mean Pearson r: {no_qg_data['Pearson_r'].mean():.4f}")
        print(f"  Mean significance: {no_qg_data['Significance_Sigma'].mean():.2f}σ")
        print(f"  Max significance: {no_qg_data['Significance_Sigma'].max():.2f}σ")
...
def create_visualizations(df_results, stats):
    ...
    ax1.axvline(x=5, color='darkred', linestyle='--', label='5σ threshold')
    ax1.set_xlabel('Significance (σ)')
