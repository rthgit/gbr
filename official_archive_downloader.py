#!/usr/bin/env python3
"""
=============================================================================
OFFICIAL ARCHIVE DOWNLOADER - Scaricatore Dati Ufficiali
=============================================================================
Scarica DATI GREZZI REALI da archivi astronomici ufficiali.
Implementa accesso diretto alle API pubbliche di NASA, ESA, e altri osservatori.

ARCHIVI UFFICIALI:
- Fermi GBM: https://heasarc.gsfc.nasa.gov/FTP/fermi/data/gbm/
- Fermi LAT: https://heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/
- Swift BAT: https://swift.gsfc.nasa.gov/archive/swift/
- MAGIC: https://magic.mpp.mpg.de/data/
- HESS: https://www.mpi-hd.mpg.de/hfm/HESS/

AUTORE: Christian Quintino De Luca (RTH Italia)
DATA: 2025-10-20
"""

import os
import sys
import requests
import numpy as np
from astropy.io import fits
from astropy.table import Table
import json
from datetime import datetime, timedelta
import time
import urllib.parse
from pathlib import Path
import re
import zipfile
import tarfile

# Configurazione encoding per Windows
try:
    sys.stdout.reconfigure(encoding='utf-8')
except Exception:
    pass

class OfficialArchiveDownloader:
    """Scaricatore di dati ufficiali da archivi astronomici."""
    
    def __init__(self):
        self.base_dir = 'official_archive_data'
        self.instrument_dirs = {
            'fermi_gbm': os.path.join(self.base_dir, 'fermi_gbm'),
            'fermi_lat': os.path.join(self.base_dir, 'fermi_lat'),
            'swift_bat': os.path.join(self.base_dir, 'swift_bat'),
            'magic': os.path.join(self.base_dir, 'magic'),
            'hess': os.path.join(self.base_dir, 'hess')
        }
        
        # Crea directory
        for dir_path in self.instrument_dirs.values():
            os.makedirs(dir_path, exist_ok=True)
        
        # URL degli archivi ufficiali
        self.archive_urls = {
            'fermi_gbm_base': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/gbm/',
            'fermi_lat_base': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/',
            'swift_bat_base': 'https://swift.gsfc.nasa.gov/archive/swift/',
            'magic_base': 'https://magic.mpp.mpg.de/data/',
            'hess_base': 'https://www.mpi-hd.mpg.de/hfm/HESS/'
        }
        
        # Parametri GRB target con trigger ID reali e URL specifici
        self.target_grbs = {
            'GRB080916C': {
                'trigger_id': '080916409',
                'trigger_time': '2008-09-16T00:12:45',
                'date': '2008-09-16',
                'redshift': 4.35,
                'max_energy': 13.2,  # GeV
                'instruments': ['fermi_gbm', 'fermi_lat'],
                'ra': 119.8,
                'dec': -56.6,
                'fermi_gbm_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/gbm/2008/09/16/',
                'fermi_lat_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/triggers/2008/09/16/'
            },
            'GRB130427A': {
                'trigger_id': '130427324',
                'trigger_time': '2013-04-27T04:57:00',
                'date': '2013-04-27',
                'redshift': 0.34,
                'max_energy': 95.0,  # GeV
                'instruments': ['fermi_gbm', 'fermi_lat', 'swift_bat'],
                'ra': 173.1,
                'dec': 27.7,
                'fermi_gbm_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/gbm/2013/04/27/',
                'fermi_lat_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/triggers/2013/04/27/',
                'swift_bat_url': 'https://swift.gsfc.nasa.gov/archive/swift/bat/trigger_20130427/'
            },
            'GRB090510': {
                'trigger_id': '090510016',
                'trigger_time': '2009-05-10T00:23:00',
                'date': '2009-05-10',
                'redshift': 0.903,
                'max_energy': 31.0,  # GeV
                'instruments': ['fermi_gbm', 'fermi_lat'],
                'ra': 333.5,
                'dec': -26.6,
                'fermi_gbm_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/gbm/2009/05/10/',
                'fermi_lat_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/triggers/2009/05/10/'
            },
            'GRB190114C': {
                'trigger_id': '190114873',
                'trigger_time': '2019-01-14T20:57:03',
                'date': '2019-01-14',
                'redshift': 0.4245,
                'max_energy': 1000.0,  # GeV
                'instruments': ['fermi_gbm', 'fermi_lat', 'magic'],
                'ra': 56.5,
                'dec': 70.8,
                'fermi_gbm_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/gbm/2019/01/14/',
                'fermi_lat_url': 'https://heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/triggers/2019/01/14/',
                'magic_url': 'https://magic.mpp.mpg.de/data/grb190114c/'
            }
        }
        
        print("🔬 OFFICIAL ARCHIVE DOWNLOADER INIZIALIZZATO")
        print("=" * 60)
        print(f"📁 Directory base: {self.base_dir}")
        print(f"🎯 GRB target: {list(self.target_grbs.keys())}")
        print("⚠️  ATTENZIONE: Scaricamento dati REALI da archivi ufficiali")
    
    def test_archive_access(self):
        """Testa l'accessibilità degli archivi ufficiali."""
        print("\n🌐 TEST ACCESSIBILITÀ ARCHIVI UFFICIALI")
        print("=" * 50)
        
        results = {}
        
        for archive_name, url in self.archive_urls.items():
            try:
                print(f"🔍 Testando {archive_name}...")
                response = requests.get(url, timeout=20)
                
                if response.status_code == 200:
                    print(f"   ✅ {archive_name}: ACCESSIBILE")
                    results[archive_name] = True
                else:
                    print(f"   ❌ {archive_name}: HTTP {response.status_code}")
                    results[archive_name] = False
                    
            except requests.exceptions.RequestException as e:
                print(f"   ❌ {archive_name}: ERRORE - {str(e)[:50]}...")
                results[archive_name] = False
            
            time.sleep(3)  # Rispetta i server
        
        return results
    
    def parse_directory_listing(self, html_content):
        """Parsa il contenuto HTML di una directory per trovare file."""
        files = []
        
        # Pattern per trovare file FITS
        fits_pattern = r'href="([^"]*\.fits)"'
        fits_matches = re.findall(fits_pattern, html_content, re.IGNORECASE)
        
        # Pattern per trovare file ZIP/TAR
        archive_pattern = r'href="([^"]*\.(?:zip|tar|gz))"'
        archive_matches = re.findall(archive_pattern, html_content, re.IGNORECASE)
        
        files.extend(fits_matches)
        files.extend(archive_matches)
        
        return files
    
    def download_fermi_gbm_official_data(self, grb_info):
        """Scarica dati ufficiali Fermi GBM."""
        grb_name = grb_info.get('grb_name', 'Unknown')
        date = grb_info['date']
        
        print(f"\n📡 SCARICANDO FERMI GBM UFFICIALE - {grb_name}")
        print("=" * 55)
        
        # URL per il giorno specifico
        year = date[:4]
        month = int(date[5:7])
        day = int(date[8:10])
        
        base_url = f"https://heasarc.gsfc.nasa.gov/FTP/fermi/data/gbm/{year}/{month:02d}/{day:02d}/"
        
        print(f"🔍 URL directory: {base_url}")
        
        downloaded_files = []
        
        try:
            # Prova a scaricare directory listing
            response = requests.get(base_url, timeout=30)
            
            if response.status_code == 200:
                print(f"   ✅ Directory accessibile")
                
                # Parsa directory listing
                files = self.parse_directory_listing(response.text)
                
                if files:
                    print(f"   📄 Trovati {len(files)} file:")
                    for file in files[:10]:  # Mostra solo i primi 10
                        print(f"      - {file}")
                    
                    # Prova a scaricare file FITS
                    for file in files:
                        if file.endswith('.fits'):
                            try:
                                file_url = base_url + file
                                print(f"   🔍 Scaricando: {file}")
                                
                                file_response = requests.get(file_url, timeout=60)
                                if file_response.status_code == 200:
                                    # Salva file
                                    filepath = os.path.join(
                                        self.instrument_dirs['fermi_gbm'], 
                                        f"{grb_name.lower()}_gbm_{file}"
                                    )
                                    
                                    with open(filepath, 'wb') as f:
                                        f.write(file_response.content)
                                    
                                    file_size_kb = len(file_response.content) / 1024
                                    print(f"   ✅ Scaricato: {filepath} ({file_size_kb:.1f} KB)")
                                    
                                    # Verifica FITS
                                    try:
                                        with fits.open(filepath) as hdul:
                                            print(f"   ✅ FITS valido: {len(hdul)} HDUs")
                                            downloaded_files.append(filepath)
                                    except Exception as e:
                                        print(f"   ⚠️  FITS non valido: {e}")
                                        os.remove(filepath)
                                        
                                else:
                                    print(f"   ❌ Errore download: HTTP {file_response.status_code}")
                                    
                            except Exception as e:
                                print(f"   ❌ Errore scaricamento {file}: {e}")
                            
                            time.sleep(2)  # Rispetta i server
                else:
                    print(f"   ℹ️  Nessun file trovato nella directory")
                    
            else:
                print(f"   ❌ Directory non accessibile: HTTP {response.status_code}")
                
        except requests.exceptions.RequestException as e:
            print(f"   ❌ Errore accesso: {str(e)[:50]}...")
        
        return downloaded_files
    
    def download_fermi_lat_official_data(self, grb_info):
        """Scarica dati ufficiali Fermi LAT."""
        grb_name = grb_info.get('grb_name', 'Unknown')
        date = grb_info['date']
        
        print(f"\n📡 SCARICANDO FERMI LAT UFFICIALE - {grb_name}")
        print("=" * 55)
        
        year = date[:4]
        month = int(date[5:7])
        day = int(date[8:10])
        
        base_url = f"https://heasarc.gsfc.nasa.gov/FTP/fermi/data/lat/triggers/{year}/{month:02d}/{day:02d}/"
        
        print(f"🔍 URL directory: {base_url}")
        
        downloaded_files = []
        
        try:
            response = requests.get(base_url, timeout=30)
            
            if response.status_code == 200:
                print(f"   ✅ Directory accessibile")
                
                files = self.parse_directory_listing(response.text)
                
                if files:
                    print(f"   📄 Trovati {len(files)} file:")
                    for file in files[:10]:
                        print(f"      - {file}")
                    
                    # Scarica file FITS
                    for file in files:
                        if file.endswith('.fits'):
                            try:
                                file_url = base_url + file
                                print(f"   🔍 Scaricando: {file}")
                                
                                file_response = requests.get(file_url, timeout=60)
                                if file_response.status_code == 200:
                                    filepath = os.path.join(
                                        self.instrument_dirs['fermi_lat'], 
                                        f"{grb_name.lower()}_lat_{file}"
                                    )
                                    
                                    with open(filepath, 'wb') as f:
                                        f.write(file_response.content)
                                    
                                    file_size_kb = len(file_response.content) / 1024
                                    print(f"   ✅ Scaricato: {filepath} ({file_size_kb:.1f} KB)")
                                    
                                    try:
                                        with fits.open(filepath) as hdul:
                                            print(f"   ✅ FITS valido: {len(hdul)} HDUs")
                                            downloaded_files.append(filepath)
                                    except Exception as e:
                                        print(f"   ⚠️  FITS non valido: {e}")
                                        os.remove(filepath)
                                        
                                else:
                                    print(f"   ❌ Errore download: HTTP {file_response.status_code}")
                                    
                            except Exception as e:
                                print(f"   ❌ Errore scaricamento {file}: {e}")
                            
                            time.sleep(2)
                else:
                    print(f"   ℹ️  Nessun file trovato nella directory")
                    
            else:
                print(f"   ❌ Directory non accessibile: HTTP {response.status_code}")
                
        except requests.exceptions.RequestException as e:
            print(f"   ❌ Errore accesso: {str(e)[:50]}...")
        
        return downloaded_files
    
    def download_swift_bat_official_data(self, grb_info):
        """Scarica dati ufficiali Swift BAT."""
        grb_name = grb_info.get('grb_name', 'Unknown')
        date = grb_info['date']
        
        print(f"\n📡 SCARICANDO SWIFT BAT UFFICIALE - {grb_name}")
        print("=" * 55)
        
        year = date[:4]
        month = int(date[5:7])
        day = int(date[8:10])
        
        base_url = f"https://swift.gsfc.nasa.gov/archive/swift/bat/trigger_{year}{month:02d}{day:02d}/"
        
        print(f"🔍 URL directory: {base_url}")
        
        downloaded_files = []
        
        try:
            response = requests.get(base_url, timeout=30)
            
            if response.status_code == 200:
                print(f"   ✅ Directory accessibile")
                
                files = self.parse_directory_listing(response.text)
                
                if files:
                    print(f"   📄 Trovati {len(files)} file:")
                    for file in files[:10]:
                        print(f"      - {file}")
                    
                    # Scarica file FITS
                    for file in files:
                        if file.endswith('.fits'):
                            try:
                                file_url = base_url + file
                                print(f"   🔍 Scaricando: {file}")
                                
                                file_response = requests.get(file_url, timeout=60)
                                if file_response.status_code == 200:
                                    filepath = os.path.join(
                                        self.instrument_dirs['swift_bat'], 
                                        f"{grb_name.lower()}_bat_{file}"
                                    )
                                    
                                    with open(filepath, 'wb') as f:
                                        f.write(file_response.content)
                                    
                                    file_size_kb = len(file_response.content) / 1024
                                    print(f"   ✅ Scaricato: {filepath} ({file_size_kb:.1f} KB)")
                                    
                                    try:
                                        with fits.open(filepath) as hdul:
                                            print(f"   ✅ FITS valido: {len(hdul)} HDUs")
                                            downloaded_files.append(filepath)
                                    except Exception as e:
                                        print(f"   ⚠️  FITS non valido: {e}")
                                        os.remove(filepath)
                                        
                                else:
                                    print(f"   ❌ Errore download: HTTP {file_response.status_code}")
                                    
                            except Exception as e:
                                print(f"   ❌ Errore scaricamento {file}: {e}")
                            
                            time.sleep(2)
                else:
                    print(f"   ℹ️  Nessun file trovato nella directory")
                    
            else:
                print(f"   ❌ Directory non accessibile: HTTP {response.status_code}")
                
        except requests.exceptions.RequestException as e:
            print(f"   ❌ Errore accesso: {str(e)[:50]}...")
        
        return downloaded_files
    
    def download_magic_official_data(self, grb_info):
        """Scarica dati ufficiali MAGIC."""
        grb_name = grb_info.get('grb_name', 'Unknown')
        
        print(f"\n📡 SCARICANDO MAGIC UFFICIALE - {grb_name}")
        print("=" * 55)
        
        base_url = "https://magic.mpp.mpg.de/data/"
        
        print(f"🔍 URL base: {base_url}")
        
        downloaded_files = []
        
        try:
            print("🔍 Cercando dati MAGIC...")
            
            # MAGIC richiede spesso registrazione e autenticazione
            print("   ℹ️  MAGIC richiede spesso registrazione per dati grezzi")
            print("   ℹ️  Dati pubblici disponibili solo per eventi specifici")
            
        except Exception as e:
            print(f"   ❌ Errore accesso MAGIC: {e}")
        
        return downloaded_files
    
    def download_all_official_data(self):
        """Scarica tutti i dati ufficiali per i GRB target."""
        print("\n🚀 DOWNLOAD COMPLETO DATI UFFICIALI")
        print("=" * 60)
        
        # Test accessibilità
        archive_status = self.test_archive_access()
        
        all_downloads = {}
        
        for grb_name in self.target_grbs.keys():
            print(f"\n🎯 PROCESSANDO {grb_name}")
            print("-" * 40)
            
            grb_info = self.target_grbs[grb_name].copy()
            grb_info['grb_name'] = grb_name
            grb_downloads = {}
            
            for instrument in grb_info['instruments']:
                try:
                    if instrument == 'fermi_gbm' and archive_status.get('fermi_gbm_base', False):
                        files = self.download_fermi_gbm_official_data(grb_info)
                        grb_downloads['fermi_gbm'] = files
                    elif instrument == 'fermi_lat' and archive_status.get('fermi_lat_base', False):
                        files = self.download_fermi_lat_official_data(grb_info)
                        grb_downloads['fermi_lat'] = files
                    elif instrument == 'swift_bat' and archive_status.get('swift_bat_base', False):
                        files = self.download_swift_bat_official_data(grb_info)
                        grb_downloads['swift_bat'] = files
                    elif instrument == 'magic':
                        files = self.download_magic_official_data(grb_info)
                        grb_downloads['magic'] = files
                        
                except Exception as e:
                    print(f"   ❌ Errore con {instrument}: {e}")
            
            all_downloads[grb_name] = grb_downloads
            
            total_files = sum(len(files) for files in grb_downloads.values())
            print(f"✅ {grb_name}: {total_files} file scaricati")
        
        # Salva metadata
        metadata = {
            'download_time': datetime.now().isoformat(),
            'downloads': all_downloads,
            'archive_status': archive_status,
            'target_grbs': self.target_grbs
        }
        
        metadata_file = os.path.join(self.base_dir, 'official_download_metadata.json')
        with open(metadata_file, 'w') as f:
            json.dump(metadata, f, indent=2, default=str)
        
        print(f"\n📋 RIEPILOGO DOWNLOAD UFFICIALE:")
        print(f"   📁 Directory: {self.base_dir}/")
        print(f"   📄 Metadata: {metadata_file}")
        print(f"   🎯 GRB processati: {len(all_downloads)}")
        
        total_files = sum(
            sum(len(files) for files in grb_downloads.values())
            for grb_downloads in all_downloads.values()
        )
        print(f"   📊 File totali scaricati: {total_files}")
        
        return all_downloads
    
    def verify_official_data_quality(self):
        """Verifica la qualità dei dati ufficiali scaricati."""
        print("\n🔍 VERIFICA QUALITÀ DATI UFFICIALI")
        print("=" * 45)
        
        verification_results = {}
        
        for instrument_dir in self.instrument_dirs.values():
            if os.path.exists(instrument_dir):
                instrument_name = os.path.basename(instrument_dir)
                print(f"\n🔍 Verificando {instrument_name}:")
                
                instrument_results = {}
                
                for filename in os.listdir(instrument_dir):
                    if filename.endswith('.fits'):
                        filepath = os.path.join(instrument_dir, filename)
                        
                        try:
                            with fits.open(filepath) as hdul:
                                # Analisi dettagliata del file
                                file_info = {
                                    'file_size_kb': os.path.getsize(filepath) / 1024,
                                    'n_hdus': len(hdul),
                                    'hdu_info': []
                                }
                                
                                for i, hdu in enumerate(hdul):
                                    hdu_info = {
                                        'hdu_index': i,
                                        'hdu_type': hdu.__class__.__name__,
                                        'has_data': hdu.data is not None
                                    }
                                    
                                    if hdu.data is not None:
                                        hdu_info['data_shape'] = hdu.data.shape
                                        hdu_info['data_dtype'] = str(hdu.data.dtype)
                                        
                                        # Cerca colonne di interesse
                                        if hasattr(hdu.data, 'dtype') and hdu.data.dtype.names:
                                            hdu_info['columns'] = list(hdu.data.dtype.names)
                                    
                                    file_info['hdu_info'].append(hdu_info)
                                
                                instrument_results[filename] = file_info
                                print(f"   ✅ {filename}: {file_info['file_size_kb']:.1f} KB, {len(hdul)} HDUs")
                                
                        except Exception as e:
                            instrument_results[filename] = {'error': str(e)}
                            print(f"   ❌ {filename}: ERRORE - {e}")
                
                verification_results[instrument_name] = instrument_results
        
        # Salva risultati verifica
        verification_file = os.path.join(self.base_dir, 'official_data_verification.json')
        with open(verification_file, 'w') as f:
            json.dump(verification_results, f, indent=2, default=str)
        
        print(f"\n📄 Verifica salvata: {verification_file}")
        
        return verification_results


def main():
    """Funzione principale per download dati ufficiali."""
    print("🔬 OFFICIAL ARCHIVE DOWNLOADER")
    print("=" * 50)
    print("Autore: Christian Quintino De Luca (RTH Italia)")
    print("Data: 2025-10-20")
    print("=" * 50)
    
    # Inizializza downloader
    downloader = OfficialArchiveDownloader()
    
    # Scarica tutti i dati ufficiali
    all_downloads = downloader.download_all_official_data()
    
    # Verifica qualità
    verification_results = downloader.verify_official_data_quality()
    
    print(f"\n🎉 DOWNLOAD UFFICIALE COMPLETATO!")
    print(f"📁 Controlla la directory '{downloader.base_dir}/' per i file scaricati")
    
    return all_downloads, verification_results


if __name__ == "__main__":
    main()

